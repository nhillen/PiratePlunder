// Complete Dice Collection Definitions - Pirate Waters Series I
// Each collection is a pre-designed combination with full visual package

import { normalizeCombo, type DiceCombo } from '../utils/diceRulesEngine';

export interface DiceCollection {
  id: string;
  name: string;
  rarity: string;
  theme?: string;
...
// Rarity pricing: Swabbie $2, Deckhand $4, Corsair $6, Captain $8
...
const RAW_COLLECTIONS: Omit<DiceCollection, 'complexity'>[] = [
  // ================= Swabbie (Common) =================
  {
    id: "bone-classic",
    name: "Bone Classic",
...
// Apply rules engine normalization to all collections
export const DICE_COLLECTIONS: DiceCollection[] = RAW_COLLECTIONS.map(collection => ({
  ...collection,
  combo: normalizeCombo(collection.combo),
  complexity: 0 // Will be calculated by rules engine
}));

// Helper functions
export function getCollectionById(id: string): DiceCollection | undefined {
  return DICE_COLLECTIONS.find(collection => collection.id === id);
}

export function getCollectionsByRarity(rarity: string): DiceCollection[] {
  return DICE_COLLECTIONS.filter(collection => collection.rarity === rarity);
}

export function getAllRarities(): string[] {
  return [...new Set(DICE_COLLECTIONS.map(collection => collection.rarity))];
}
